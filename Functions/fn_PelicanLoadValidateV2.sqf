...
switch true do {
	...
	{
		_veh attachTo [_pelican,[0,4,-4]]; 
		_loaded = true;  
	};
	case (_veh isKindOf "TG_APC_GDI_01"):
	{
		_veh attachTo [_pelican,[0, 6, -6]];
		_loaded = true;  
	};	
	...
	{
		_veh attachTo [_pelican,[0.15,4,-3]]; 
		_loaded = true;
	};
	...
	{
		_veh attachTo [_pelican,[0.15,-5,-3]];  
		_loaded = true;  
	};
...
if (_loaded) then 
{
    _pelican setVariable ["OPTRE_Pelican_AttachedToVehiclesEffect", (_pelican getVariable ["OPTRE_Pelican_AttachedToVehiclesEffect", []]) + [vehicle player], true];
    titleText ["-------------------------------------------<br/><t color='#ff0000' size='1.5'>MAGLOCK SUCCESSFUL!</t><br/>-------------------------------------------<br/>", "PLAIN DOWN", -1, true, true];
    playSound "FD_Finish_F";
        if (_pelican getVariable ["53rd_Pelican_KilledEvent", -1] >= 0) exitWith {};
    private _EH = _pelican addEventHandler ["Killed",
    {
        ...
        _pelican setVariable ["53rd_Pelican_KilledEvent", _EH]
} else {
